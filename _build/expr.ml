...
and is_matching expr env motif = try true, tri_filtre false env motif expr with _ -> false,[]  


and tri_filtre recursif env motif = function
| ValRef e1          -> begin match eval e1 env with
    | VRef k -> filtre_val env ref_memory.(k) motif
    | _ -> raise (EvalError "ValRef error (type should be ref)")
  end
  | Var cle            -> begin match cle with
      | MNom nom -> filtre_val env (trouver_env nom env) motif
      | _       -> []
    end
  | e -> filtre e recursif env motif
...
and filtre expr recursif env motif = fusion_env env (match motif with
  ...
  | MCouple (m1,m2) -> begin match expr with
        ...
        | _ -> raise (EvalError "Filtre : motif impossible couple")
      end
  | MCons (m1,m2) -> begin match expr with
        ...
        | _ -> raise (EvalError "Filtre : motif impossible cons")
      end
  | MEmptyList -> begin match expr with
        | Var (MNom v) -> begin match trouver_env v env with
              | VList [] -> []
              | _ -> raise (EvalError "Filtre : motif impossible empty1")
            end
        | EmptyList -> []
        | _ -> raise (EvalError "Filtre : motif impossible empty2")
      end
  | MUnit -> begin match eval expr env with
        | VUnit -> []
        | _ -> raise (EvalError "Filtre : motif impossible unit")
      end
...
and eval e env = 
  ...
  | Let (recursif,var,e1,global,e2) -> if not global then Options.in_dscolon := true;
                                       ...
                                       eval e2 (tri_filtre recursif env var e1)
