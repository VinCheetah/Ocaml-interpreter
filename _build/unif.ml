...
let rec replace (x, new_x) term =
  match term with
  | Var (v,t1,b1) -> if v = x then new_x else Var (v,replace (x,new_x) t1,b1)
...
let rec unify pb = incr compt;
  ...
  | x :: pb' -> begin
    match x with
    | Var (v, t, b), _ when type_fixed t || !compt >= 1000 -> if appear v t then raise Not_unifyable else (unify (List.map (fun (t1,t2) -> (replace (v, t) t1), replace (v, t) t2) pb')) @ (if true then [(Var (v,None,false), t)] else [])
    | _, Var (v, t, b) when type_fixed t || !compt >= 1000 -> if appear v t then raise Not_unifyable else (unify (List.map (fun (t1,t2) -> (replace (v, t) t1), replace (v, t) t2) pb')) @ (if true then [(Var (v,None,false), t)] else [])
