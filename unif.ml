...
let rec appear v t = 
  match t with
  | Var (v',_,_)    -> v = v'
  | T (TFun (a,corps)) -> appear v a || appear v corps
  | T (TRef r)         -> appear v r
  | T (TTuple (a,b))   -> appear v a || appear v b
...
let rec unify pb =
  ...
  | x :: pb' -> begin
    match x with
    | Var (v, t, b), _ when type_fixed t || !compt >= compt_max -> if appear v t then raise Not_unifyable else Var (v, t, b) :: (unify (List.map (fun (t1,t2) -> (replace (v, t) t1), replace (v, t) t2) pb')) 
    | _, Var (v, t, b) when type_fixed t || !compt >= compt_max -> if appear v t then raise Not_unifyable else Var (v, t, b) :: (unify (List.map (fun (t1,t2) -> (replace (v, t) t1), replace (v, t) t2) pb'))
